...
const std::string DEFAULT_CONFIG =
        ...
        "updater: 1;\n"
        "git_repo: https://github.com/TureBentzin/Heinrich.git;\n"
        "bot_debug: 0;\n";
#else
const std::string DEFAULT_CONFIG =
        "java_bin: java;\n"
...
int main(int argc, char **argv) {
    ...
    std::cout << "Welcome to the Heinrich Launcher!" << std::endl;
...
// check if the file exists and is readable. if not, create it.
    ...
    if (config.updater && enable_updater) {
        ...
        if (checkForCommand("git")) {
            ...
            if (checkForCommand("mvn")) {
                ...
                std::string copy_command = "cp source/target/Heinrich.jar " + config.jar;
    ...
    if (!config.jar.ends_with(".jar")) {
        std::cout << "Jar file must end with .jar! Please make sure Heinrich is installed correctly!" << std::endl;
        return 1;
    }
    if (!jar_file.is_open()) {
        std::cout << "Jar file not found! Please make sure Heinrich is installed correctly!" << std::endl;
        return 1;
    }
    jar_file.close();
